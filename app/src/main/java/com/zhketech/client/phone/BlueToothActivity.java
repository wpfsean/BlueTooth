...
public class BlueToothActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private BlueToothActivity.BlueToothStateReceiver mReceiver;//广播接收器
    private BlueToothActivity.ConnectThread mConnectThread; //客户端线程
    private BlueToothActivity.AcceptThread mAcceptThread; //服务端线程
    ...
    private void registerRec() {
        //3.注册蓝牙广播
        mReceiver = new BlueToothActivity.BlueToothStateReceiver();
        ...
        switch (view.getId()) {
            case R.id.open:
                ...
                if (mAcceptThread == null && mBluetoothAdapter != null) {
                    mAcceptThread = new BlueToothActivity.AcceptThread();
                    mAcceptThread.start();
    ...
    class ConnectThread extends Thread {
        ...
        public void run() {
            if (mDevice != null) {
                try {
                    //获取套接字
                    @SuppressLint("MissingPermission") BluetoothSocket temp = mDevice.createInsecureRfcommSocketToServiceRecord(UUID.fromString(BT_UUID));
